...
variable "bucket_location" {
  description = "Cloud Storage bucket location for the state bucket."
  type        = string
  default     = "us-central1"
...
variable "state_bucket_prefix" {
  description = "Lower-case prefix for the globally unique state bucket name. A random suffix is appended."
  type        = string
  default     = "tf-gcp-practice-state"

  validation {
    condition     = can(regex("^[a-z0-9][a-z0-9-]{1,50}[a-z0-9]$", var.state_bucket_prefix))
    error_message = "state_bucket_prefix must be 3 to 52 lower-case letters, numbers, or hyphens and cannot end with a hyphen."
  }
}

variable "force_destroy_state_bucket" {
  description = "Whether Terraform may delete the state bucket even when it contains objects. Keep false unless intentionally cleaning up."
  type        = bool
  default     = false
}
