...
from common.bigquery_grain_metadata_sql import (
...
class BigQueryGrainMetadataSqlTest(unittest.TestCase):
    def test_raw_external_table_sql_keeps_info_json_and_string_keys(self):
        ...
        self.assertIn(
            "`example-study-proj.dl_bloomberg_data.raw_grain_catalog`",
            sql,
        )
    ...
    def test_merge_sql_keys_on_dim_grains_pair_and_asserts_uniqueness(self):
        ...
        self.assertIn(
            "MERGE `example-study-proj.dl_bloomberg_data.dim_grain_metadata`",
            sql,
        )
        self.assertIn(
            "`example-study-proj.dl_bloomberg_data.raw_grain_catalog`",
        ...
        # The merge key must mirror dim_grains so the join stays one to one.
