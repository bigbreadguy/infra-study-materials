...
One manual-trigger DAG snapshots the catalog info object for every enabled
grain target across every category config file, then merges the BigQuery
dim_grain_metadata table. dim_grain_metadata keys on the same (id, name)
pair as dim_grains, so the two tables join one to one.
...
selection rules live in common.grain_catalog. Each emitted row carries the
grain target's dataset_id (not the catalog's process.datasetId, which can
diverge) so the dim_grains join always holds.

Grain target config files are local-only (gitignored) and live in:
  dags/local/grain_targets/<category>.json

Required Airflow Variables:
- mongo_conn_id, mongo_database_name, mongo_catalog_collection_name
- gcp_conn_id, gcs_bucket_name, gcs_raw_prefix, gcs_impersonation_chain
- bigquery_project_id, bigquery_dataset_id, bigquery_region,
...
from common.bigquery_grain_metadata import (
...
from common.grain_catalog import (
...
with DAG(
    dag_id="mongo-grain-metadata-ingestion",
    description=(
    ...
    tags=["mongo", "gcs", "bigquery", "catalog"],
) as dag:
    @task()
